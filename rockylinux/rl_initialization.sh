...
EOF
    ...
    if [[ "$IS_LXC_CONTAINER" != "true" ]] && [[ -f /etc/default/grub ]] && ! grep -q 'selinux=0' /etc/default/grub; then
        sed -i 's/\(GRUB_CMDLINE_LINUX="[^"]*\)"/\1 selinux=0"/' /etc/default/grub
        if [[ -d /sys/firmware/efi ]]; then
            grub2-mkconfig -o /boot/efi/EFI/rocky/grub.cfg &>/dev/null || true
        else
...
EOF
    ...
    if [[ "$IS_LXC_CONTAINER" == "true" ]]; then
        print_info "LXC container detected — firewalld skipped (firewall managed by host)"
    ...
    if [[ "$current_selinux" == "Disabled" ]]; then
        ...
        if [[ "$IS_LXC_CONTAINER" == "true" ]]; then
            print_info "LXC container: SELinux enforcement is controlled by the host kernel"
            print_info "GRUB kernel args cannot be set inside a container — SELinux state depends on host policy"
        else
...
EOF
    ...
    if [[ "$IS_LXC_CONTAINER" == "true" ]]; then
        print_info "LXC container detected — disk expansion skipped (storage managed by host LXD pool)"
    fi
    ...
    if [[ "$IS_LXC_CONTAINER" != "true" ]] && [[ "$_root_source" == /dev/mapper/* ]]; then
        _pv_part=$(pvs --noheadings -o pv_name 2>/dev/null | awk '{print $1}' | head -1)
        _is_lvm=true
    elif [[ "$IS_LXC_CONTAINER" != "true" ]]; then
        _pv_part="$_root_source"
    fi

    if [[ "$IS_LXC_CONTAINER" != "true" ]] && [[ -n "$_pv_part" && -b "$_pv_part" ]] && command -v parted &>/dev/null; then
